...
=== ToDo
* add initial index action for deployments --> redirect to show.
* show deployment page and give the stats we want -- adding model methods.
* refactor into access_token authorized request (returns true or raises the error)
* show alerts from the deployment
* allow hook for deployments
* show action heatmap graph
* write server client
