...
class ProtoHybi07
  attr_reader :connected, :close_received
  ...
  def make_handshake(host, protocols, extensions, addl_headers=[])
    init
    @handshake = make_handshake_key
    str = "GET / HTTP/1.1\r\n"
    str += "Host: #{host}\r\nUpgrade: websocket\r\nConnection: Upgrade\r\n"
    str += addl_headers.collect {|k,v| "#{k}: #{v}\r\n"}.join
  ...
  def valid_protocol_response?(headers, protocols)
    return true if protocols.size == 0

    h = extract_headers(headers[1..headers.size])
    return protocols.include?(h['Sec-WebSocket-Protocol'])
  end
...
#        puts "last - #{last_frame} #{idx1}-#{idx1+idx2}: #{data[idx1,idx2]}"
  ...
  def mask_payload(payload)
    if !@masking_disabled
      ...
      payload.each_byte do |byte|
        idx = i % 4
        masked_payload << (byte ^ masking_key[idx].ord)
        i += 1
      end
  ...
  def create_frame(opcode, d, last_frame = true)
    ...
    mask_key = mask_key.force_encoding("ASCII-8BIT")
    masked_payload = masked_payload.force_encoding("ASCII-8BIT")
